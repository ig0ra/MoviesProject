
//
//  GetTopRatedMoviesUseCaseTests.swift
//  MoviesTests
//
//  Created by Igor O on 05.10.2025.
//

import XCTest
@testable import Movies

final class GetTopRatedMoviesUseCaseTests: XCTestCase {

    private var sut: DefaultGetTopRatedMoviesUseCase!
    private var mockMovieRepository: MockMovieRepository!

    override func setUp() {
        super.setUp()
        mockMovieRepository = MockMovieRepository()
        sut = DefaultGetTopRatedMoviesUseCase(movieRepository: mockMovieRepository)
    }

    override func tearDown() {
        sut = nil
        mockMovieRepository = nil
        super.tearDown()
    }

    @MainActor
    func test_execute_whenRepositoryReturnsSuccess_shouldReturnPagedResponse() async throws {
        // Given
        let expectedResponse = PagedResponse<Movie>(page: 1, results: [createMockMovie()], totalPages: 1, totalResults: 1)
        mockMovieRepository.topRatedMoviesResult = .success(expectedResponse)
        
        // When
        let response = try await sut.execute(page: 1)

        // Then
        XCTAssertEqual(response.page, expectedResponse.page)
        XCTAssertEqual(response.results.count, expectedResponse.results.count)
        XCTAssertEqual(response.results.first?.id, expectedResponse.results.first?.id)
    }

    func test_execute_whenRepositoryReturnsFailure_shouldReturnError() async {
        // Given
        let expectedError = TestError.genericError
        mockMovieRepository.topRatedMoviesResult = .failure(expectedError)
        
        // When
        do {
            _ = try await sut.execute(page: 1)
            XCTFail("Expected failure but got success")
        } catch {
            XCTAssertEqual(error as? TestError, expectedError)
        }
    }
    
    // MARK: - Helpers
    
    private func createMockMovie() -> Movie {
        return Movie(id: 1, title: "Movie A", posterPath: nil, overview: "", releaseDate: nil, genreIds: [], voteAverage: 0)
    }
}

